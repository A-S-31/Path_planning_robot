...
try:
    while True:
        ...
        if dist < DISTANCE_THRESHOLD:
            ...
            time.sleep(1)
            stop()
            print("Checking first left direction and then right direction\n")
            
            if distl>DISTANCE_THRESHOLD and distr<DISTANCE_THRESHOLD:
                left()
                time.sleep(1)
                stop()
            elif distr>DISTANCE_THRESHOLD and distl<DISTANCE_THRESHOLD:
                right()
                time.sleep(1)
                stop()
            elif distr>DISTANCE_THRESHOLD and distl>DISTANCE_THRESHOLD:
